...
The weekly and monthly prompts must begin with `research_capabilities`, run `python3 scripts/atlas.py freshness`, record missing credentials and provider fallbacks, and keep `.harness/inbox/` uncommitted. For model-dependent questions they search the rolling 18-month window and current system generations first, then retain older work for mechanisms and stronger designs. They use the thin intent-level MCP and may delegate independent read-heavy work to the project-scoped scout, evidence, contrarian, and synthesis agents; one integrating agent owns canonical edits. At least once per month the synthesis audits source-lane diversity—including public social discourse and podcasts—and identifies which perspectives are absent; it does not add sources merely to meet a quota.
